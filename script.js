// ===== CONFIGURACIÓN DE SKULPT (INTÉRPRETE PYTHON) =====
function configureSkulpt(outputId) {
  Sk.pre = outputId;
  Sk.configure({
...
function runPythonCode(editorId, outputId) {
  ...
  if (!code.trim()) {
    output.innerHTML = "⚠️ ¡Escribe algo de código primero!";
    output.classList.add("error");
    return;
  }

  // Configurar Skulpt
  configureSkulpt(outputId);
    ...
    promise.then(
      function (module) {
        ...
        if (output.innerHTML.trim() === "") {
          output.innerHTML = "✅ ¡Código ejecutado correctamente! (Sin salida)";
          output.classList.add("success");
        }

        // Añadir mensaje de felicitación
        setTimeout(() => {
          showCelebration("🎉 ¡Excelente trabajo, marinero!");
        }, 500);
      },
      function (err) {
        // Error
        output.innerHTML = "❌ Error: " + err.toString();
        output.classList.add("error");
      }
    );
  } catch (err) {
...
// ===== LIMPIAR EDITOR =====
function clearEditor(editorId, outputId) {
  document.getElementById(editorId).value = "";
  const output = document.getElementById(outputId);
  output.innerHTML = "📝 Editor limpio - ¡Escribe tu código aquí!";
  output.className = "editor-output";
}
...
// ===== FUNCIONES DE EJEMPLOS DEMOSTRATIVOS =====
function runForExample() {
  ...
  islas.forEach((isla) => {
    result += `🏴‍☠️ Llegamos a ${isla}!\n`;
    result += `🔍 Buscando tesoros...\n`;
    result += `💰 ¡Tesoro encontrado!\n`;
    result += `---\n`;
  });

  result += `\n🎉 ¡Misión completada! Visitamos ${islas.length} islas y encontramos muchos tesoros!`;

  typeWriter(output, result, 50);
}
